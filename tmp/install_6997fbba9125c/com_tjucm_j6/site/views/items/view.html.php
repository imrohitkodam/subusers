...
class TjucmViewItems extends HtmlView
{
	...
	public function display($tpl = null)
	{
		...
		$this->sortableFields = array('text', 'number', 'checkbox', 'textarea',
		'textareacounter', 'calendar', 'email', 'radio', 'single_select', 'itemcategory',
		'cluster', 'ownership');
		...
		if (!$user->authorise('core.manageall', 'com_cluster'))
		{
			...
			if (!RBACL::check($user->id, 'com_cluster', 'core.viewitemlist.' . $this->ucmTypeId, 'com_tjucm'))
			{
		...
		// DPE Hack need to send MR to Core This issue will occure when we select custom layout for list page
		$this->layout = $input->get('layout', '', 'STRING');

		if (isset($this->ucmTypeParams->list_layout) && !empty($this->ucmTypeParams->list_layout) && !$this->layout)
		{
			$this->setLayout($this->ucmTypeParams->list_layout);
		}

		// Set layout for masterlist DPE hack this clients will be replaced with configs
		$input         = Factory::getApplication()->input;
		$isMasterlist  = $input->get("isMasterlist", 0, "INT");
		$isReverseList = $input->get("reverselist", 0, "INT");
		...
		if ($isMasterlist && in_array($this->client, $masterListClients))
		{
			$this->setLayout('shaper_helix3:coredatalist');
		}
		elseif ($isReverseList && in_array($this->client, $reverseListClients))
		{
			$this->setLayout('shaper_helix3:reverselist');
		}
