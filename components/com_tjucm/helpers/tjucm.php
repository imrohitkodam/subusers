...
class TjucmHelpersTjucm
{
	...
	public static function getModel($name)
	{
		...
		$model = $mvcFactory->createModel($name, 'Site');
	...
	public static function getLanguageConstantForJs()
	{
		...
		Text::script('JLIB_HTML_PLEASE_MAKE_A_SELECTION_FROM_THE_LIST', true);
	}
					...
					foreach ($menuItems as $menuItem)
					{
						...
						if (!empty($menuParams))
						{
							...
							$mvcFactory = Factory::getApplication()
								->bootComponent('com_tjucm')
								->getMVCFactory();
							$ucmTypeTable = $mvcFactory->createTable('Type', 'Administrator', array('dbo', $db));
