...
/* DPE Hack Variable to store if form is submited for final save and Close*/
var tjUcmFormFinalSaveClose = 0;
...
jQuery(window).on('load', function () {
	...
	// DPE hack for auto save
	var isROPForm = Number(jQuery('#isROPForm').val());
	...
	if (tjUcmCurrentDraftSaveState === 1 || isROPForm === 1) {
		...
		if (tjUcmAllowAutoSave == 1) {
			...
			/* Save form values - Use only 'change' event and add debouncing to prevent multiple calls */
			if (!jQuery("#item-form").data('autosave-listener-attached')) {
				// Shared timeout object for all fields
				var saveFieldTimeout = {};
				jQuery("#item-form").data('saveFieldTimeout', saveFieldTimeout);
				
				jQuery("#item-form").on("change", ":input", function (e) {
					// Skip if this change was triggered programmatically (e.g., by chosen:updated)
					if (e.isTrigger && e.originalEvent === undefined) {
						return;
					}
					
					if (tjUcmCurrentAutoSaveState) {
						var fieldId = jQuery(this).attr('id') || jQuery(this).attr('name') || 'default';
						var timeouts = jQuery("#item-form").data('saveFieldTimeout') || {};
						...
						// Debounce: wait 300ms before saving to prevent multiple rapid calls
						timeouts[fieldId] = setTimeout(function() {
							tjUcmItemForm.onUcmFormChange(this);
							delete timeouts[fieldId];
							jQuery("#item-form").data('saveFieldTimeout', timeouts);
						}.bind(this), 300);
				...
				/* To save calendar field value - use event delegation */
				jQuery("#item-form").on("blur", ".field-calendar input:text", function () {
					if (tjUcmCurrentAutoSaveState) {
						var fieldId = jQuery(this).attr('id') || jQuery(this).attr('name') || 'calendar-default';
						var timeouts = jQuery("#item-form").data('saveFieldTimeout') || {};
				...
				jQuery("#item-form").data('autosave-listener-attached', true);
			}
			...
			if (jQuery("#item-form .wf-editor-container").length > 0) {
				...
				setTimeout(function () {
					jQuery("#item-form .mceIframeContainer iframe").each(function () {
						...
						tjUcmJCEFieldIds[tjucmJceIframeIndex] = iframeContent;
					});
				}, 2000);
			}
	...
	jQuery(document).on('subform-row-add', function (event, row) {
		var tjucmSubFormCount = jQuery(event.detail.row).attr('data-group').replace(jQuery(event.detail.row).attr('data-base-name'), "");

		/* If there is any editor field in sub-form then add its reference in variable tjUcmTinyMCEFieldIds*/
		if (jQuery(event.detail.row).find('.js-editor-tinymce textarea')) {
			var tjUcmIdOfEditorFieldInSubForm = jQuery(event.detail.row).find('.js-editor-tinymce textarea').attr('id');
		...
		if (jQuery(event.detail.row).find('.wf-editor-container textarea')) {
			var tjUcmIdOfEditorFieldInSubForm = jQuery(event.detail.row).find('.wf-editor-container textarea').attr('id');
		...
		jQuery.each(tjucmRelatedFieldUpdatedOptions, function (index, value) {
			if (value.templateId) {
				...
				jQuery(event.detail.row).find("#" + tjucmNewTemplateId).html('');
				jQuery.each(value.options, function (i, val) {
					jQuery(event.detail.row).find("#" + tjucmNewTemplateId).append('<option value="' + val.value + '">' + val.text + '</option>');
				});

				// Prevent triggering change event when updating chosen dropdown
				jQuery(event.detail.row).find("#" + tjucmNewTemplateId).trigger("chosen:updated", [true]);
			}
		});
	});
...
var tjUcmItemForm = {
	...
	onUcmFormChange: function (fieldObj) {
		...
		if (jQuery(fieldObj).data('saving-in-progress')) {
			return false;
		}
		
		/* Mark field as saving to prevent duplicate calls */
		jQuery(fieldObj).data('saving-in-progress', true);
	...
	initUcmFormFieldDataSave: function (fieldObj, tjUcmParentClient, tjUcmParentRecordId) {
		...
		if (jQuery(fieldObj).parent().parent().parent().attr('data-base-name') !== undefined || jQuery(fieldObj).parent().parent().parent().parent().attr('data-base-name') !== undefined || jQuery(fieldObj).parent().parent().parent().parent().parent().attr('data-base-name') !== undefined || jQuery(fieldObj).parent().parent().parent().parent().parent().parent().attr('data-base-name') !== undefined) {
			...
			if (tjucmRecordId == '') {
				...
				if (tjucmClient == 'com_tjucm.ropdataflow') {
					tjUcmItemFormData.append('PreElementValue', jQuery('#' + tjUcmCurrentFieldId).attr('data-prev-element-id'));
				}
	...
	afterDataSave: function (error, response) {
		...
		/* Clear the saving flag from all fields to allow future saves */
		jQuery("#item-form :input").removeData('saving-in-progress');
		...
		// DPE hack
		if (response.data && (tjUcmFormFinalSaveClose || tjUcmFormFinalSave)) {
			jQuery("#tjucm-auto-save-disabled-msg").show();
		...
		if (response.data.success && jQuery.trim(response.data.redirectUrl) != '' && response.data.redirectUrl != undefined && jQuery('#documentType').val() != 'json' && tjUcmFormFinalSaveClose) {
			setTimeout(function () {
				window.location.href = response.data.redirectUrl;
			}, 2000);
		}
		// DPE  - Hack - End - to redirect on list view
	},
	renderResponseMessages: function (response) {
		if (response != null) {
			if (response.message !== null) {
				...
				jQuery("html, body").animate({
					scrollTop: jQuery("#system-message-container").position().top - 300
				}, "slow");
			}
	...
	updateRelatedFieldsOptions: function (tjUcmParentClient, tjUcmParentRecordId) {
		...
		var tjUcmUpdateRelatedFieldsOptions = function (error, response) {
			...
			jQuery.each(response.data, function (index, value) {
				...
				// Prevent triggering change event when updating chosen dropdown
				jQuery("#" + value.elementId).trigger("chosen:updated", [true]);
			});
		};
	...
	saveUcmFormData: function () {
		...
		if (event === undefined) {
			...
			if (event.target.id == 'tjUcmSectionDraftSave' || event.target.id == 'tjUcmSectionFinalSave' || event.target.id == 'tjUcmSectionFinalSaveClose') {
		...
		// DPE  - Hack - Start - to display custom message while saving form
		var customConfirmMsg = jQuery('#custForm').val();
		var confirmMessage = Joomla.JText._("COM_TJUCM_ITEMFORM_SUBMIT_ALERT");

		if (customConfirmMsg == '1') {
			confirmMessage = Joomla.JText._("COM_TJUCM_ITEMFORM_SUBMIT_DPE_CONFIRMATION");
		}
		// DPE  - Hack - End

		// DPE  - Hack for Save and close i.e tjUcmSectionFinalSaveClose
		if (tjUcmFormSubmitCallingButtonId == 'tjUcmSectionFinalSave' || tjUcmFormSubmitCallingButtonId == 'tjUcmSectionFinalSaveClose') {
		...
		// DPE hack dont Scroll to Top in case of field click auto save
		if (!tjUcmSaveFormInDraftWithNoMsg == 1) {
			jQuery("html, body").animate({
				scrollTop: jQuery("#system-message-container").position().top
			}, "slow");

			if (jQuery("#tmplPopUp").val()) {
				var timmer = 1;
		...
		tjUcmItemForm.getUcmParentRecordId(tjUcmSaveRecordAsDraft, function () {
			...
			// DPE hack
			if (tjUcmFormSubmitCallingButtonId == 'tjUcmSectionFinalSaveClose') {
				tjUcmFormFinalSaveClose = 1;
			}
	...
	getRelatedFieldOptions: function (relatedFieldId, fieldId, clusterId) {
		...
		var tjUcmUpdateRelatedFieldsOptions = function (error, response) {
			...
			// Prevent triggering change event when updating chosen dropdown
			jQuery('#' + relatedFieldId).trigger("chosen:updated", [true]);
		};

		FieldsData.SelectedValues = jQuery('#' + relatedFieldId).val();

		if (jQuery.trim(fieldId) != '' && fieldId != 'undefined') {
			jQuery('#' + relatedFieldId + ', .chosen-results').empty();
		...
		jQuery(item_basic_form).ajaxSubmit({
			...
			success: function (data) {
				...
				if (returnedData.data !== null) {
					...
					if ('save' == status) {
						jQuery("#tjUcmSectionFinalSaveClose").attr("disabled", "disabled");
				...
				jQuery('#tjUcmSectionFinalSaveClose').attr('disabled', false);
...
// DPE Hack to get the editor Data

function editorcontent() {
	jQuery("iframe").each(function () {
		try {
			// Check if iframe is same-origin (no cross-origin issues)
			var iframeDocument = this.contentDocument || this.contentWindow.document;

			if (iframeDocument) {
				// Get all elements with the class 'mce-content-body' inside the iframe
				jQuery(iframeDocument).find(".mce-content-body").each(function () {
					var dataId = jQuery(this).attr("data-id");
					var textContent = jQuery(this).html().trim();
					jQuery("#" + dataId).text(textContent);
				});
			}
		} catch (error) {
			// If cross-origin error occurs, request data via postMessage
			console.warn("Cross-origin access blocked. Using postMessage.");
			this.contentWindow.postMessage({ type: "getEditorContent" }, "*");
		}
	});
}
...
window.addEventListener("message", function (event) {
	// Allow messages only from trusted sources
	if (!event.origin.includes("https://dataprotection.education") && !event.origin.includes("https://qa.dataprotection.education")) {
		return;
	}

	if (event.data.type === "editorContent") {
		jQuery("#" + event.data.dataId).text(event.data.content);
	}
}, false);
