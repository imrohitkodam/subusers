...
class TjucmControllerItem extends BaseController
{
	...
	public function __construct()
	{
		...
		$this->client  = Factory::getApplication()->getInput()->get('client');
		$this->created_by  = Factory::getApplication()->getInput()->get('created_by');
			...
			if (!empty($this->menuparams))
			{
				...
				if (!empty($this->ucm_type))
				{
					...
					$mvcFactory = Factory::getApplication()
						...
						->getMVCFactory();
					$ucmTypeTable = $mvcFactory->createTable('Type', 'Administrator', array('dbo', Factory::getDbo()));
		...
		$mvcFactory = Factory::getApplication()
			->bootComponent('com_tjucm')
			->getMVCFactory();
		$tjUcmModelType = $mvcFactory->createModel('Type', 'Administrator');
