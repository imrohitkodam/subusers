...
class TjucmModelItemForm extends AdminModel
{
	...
	protected function populateState()
	{
		...
		// Dpe Hack
			...
			{
				// Load the parameters.
					$params       = $app->getParams('com_tjucm');
	...
	public function &getData($id = null)
	{
		...
		if (empty($id))
		{
			...
			//DPE hack as is always se to default menu id if  id is not set.
			$app = Factory::getApplication();
			$menu = $app->getMenu();

			if ($menu->getDefault()->query['id'] == $id)
			{
				$id = 0;
			}
		}
	...
	public function save($data)  
	{
		...
		$curDate = $curDate->toSQL();
		
		$data['ordering']    = empty($data['ordering'])?0:$data['ordering'];
		$data['category_id'] = empty($data['category_id'])?0:$data['category_id'];
		$data['checked_out'] = empty($data['checked_out'])?Factory::getUser()->id:$data['category_id'];
		$data['draft']       = empty($data['draft'])?0:$data['draft'];
		$data['modified_by'] = empty($data['modified_by'])?Factory::getUser()->id:$data['modified_by'];
		$data['modified_date'] = empty($data['modified_date'])? $curDate:$data['modified_date'];
	...
	public function delete(&$contentId)
	{
		...
		// DPE Hack start to allow assignee user to delete the subform records
		// If both permissions are failed then check user is assignee
		if ($canDelete == null && $canDeleteown == null)
		{
			...
			// $table->parent_id is a ucm content id
			$assignedUsers = $dpeMainHelper->getFieldValues($user->id, $table->parent_id);

			if (!empty($assignedUsers) || TjucmAccess::canEdit($ucmTypeId, $contentId))
			{
				$canDelete = true;
			}
		}
	...
	/**
	 * Recursive function to store textarea values before validation
	 *
	 * @param   array   $data            Form data.
	 * @param   array   &$textareaValues Reference array to store textarea values.
	 * @param   string  $parentKey       Unique key for nested fields.
	 * @param   object  $getFieldId      Field object to retrieve parameters.
	 *
	 * @return  void
	 */
	private function storeTextareaFields($data, &$textareaValues, $parentKey = '', $getFieldId) 
	{
		foreach ($data as $key => $formdata) {
			$fullKey = $parentKey ? "{$parentKey}.{$key}" : $key;
			
			if (is_array($formdata)) {
				// Recursive call for subforms
				$this->storeTextareaFields($formdata, $textareaValues, $fullKey, $getFieldId);
				continue;
			}
			
			if (!$getFieldId->load(['name' => $key])) {
				continue;
			}
			
			$fieldParams = isset($getFieldId->params) ? json_decode($getFieldId->params) : null;
			
			if ($getFieldId->type === "textarea" && isset($fieldParams->texteditmode) && $fieldParams->texteditmode == 1) {
				$textareaValues[$fullKey] = $formdata;
			}
		}
	}

	/**
	 * Function to restore textarea values after validation
	 *
	 * @param   array  &$validatedData  Reference to validated data array.
	 * @param   array  $textareaValues  Stored textarea values.
	 *
	 * @return  void
	 */
	private function restoreTextareaFields(&$validatedData, $textareaValues) 
	{
		foreach ($textareaValues as $key => $value) {
			$keys = explode('.', $key);
			$temp = &$validatedData;
			
			foreach ($keys as $innerKey) {
				if (!isset($temp[$innerKey])) {
					continue 2; // Skip if key doesn't exist
				}
				$temp = &$temp[$innerKey];
			}
			
			$temp = $value; // Restore original textarea value
		}
	}
	 ...
	 *
	 * @param   object  $form   Form object containing validation rules.
	 * @param   array   $data   Data to be validated.
	 * @param   array   $group  Optional validation group.
	 *
	 * @return  mixed   Validated data array or false on failure.
	 */
	public function validate($form, $data, $group = null) 
	{
		...
		// Store textarea values before validation
		$textareaValues = [];
		$this->storeTextareaFields($data, $textareaValues, '', $getFieldId);
		
		// Perform validation
		$validatedData = parent::validate($form, $data, $group);
		
		// Restore textarea values after validation
		if ($validatedData !== false) {
			$this->restoreTextareaFields($validatedData, $textareaValues);
		}
